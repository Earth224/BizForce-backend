...
// Selected alongside the amount so the sum can check that every row it is about
// to add is denominated in the same thing. Added by migration 080; nullable,
// and null is treated as its own distinct currency rather than as a match for
// anything, because "unspecified" is not evidence of agreement.
const REVENUE_CURRENCY_COLUMN = "currency";
  ...
  for (var page = 0; page < REVENUE_MAX_PAGES; page++) {
    ...
    try {
      result = await supabase
        .from(REVENUE_TABLE)
        .select(REVENUE_AMOUNT_COLUMN + "," + REVENUE_CURRENCY_COLUMN)
...
async function gatherReviewMetrics({ supabase, userId, start, end }) {
  ...
  if (revenueResult && revenueResult.error) {
    ...
    /* ONE CURRENCY, OR NO TOTAL.
       Every distinct value in the period's currency column, with null kept as
       its own distinct value rather than folded in with anything — an
       unspecified currency is not evidence that it agrees with the others.
       Collected over every row, including rows whose amount is null, because a
       row's currency is a fact about the period regardless of whether its
       amount contributes to the sum. */
    var currencies = [];
    for (var c = 0; c < rows.length; c++) {
      var currency = rows[c] ? rows[c][REVENUE_CURRENCY_COLUMN] : null;
      var normalised = (currency === null || currency === undefined) ? null : String(currency);
      if (currencies.indexOf(normalised) === -1) {
        currencies.push(normalised);
      }
    }

    if (currencies.length > 1) {
      /* REFUSED, not filtered and not summed.
         Adding numbers denominated in different units does not produce a total
         — it produces a figure in no unit at all, which is worse than an
         unknown because it looks like an answer.

         Filtering to one currency is the tempting alternative and is worse
         still: it would silently drop the others and return a lower bound
         wearing the shape of a complete total, with nothing in the number or
         beside it to say a subset was chosen. This module already carries one
         lower bound it genuinely cannot avoid — the unattributed-revenue blind
         spot documented at the revenue read, where rows with a null user_id
         match no user — and that one is documented precisely because it cannot
         be refused. This one can be, so it is.

         The same refusal the page ceiling makes, and reported the same way: an
         unknown figure named in `unreadable`, which the prompt renders as
         unavailable-and-not-zero and the page renders as an unavailable cell
         rather than a number nobody should trust. */
      console.error("[selfReview] revenueTotal spans " + currencies.length +
        " currencies for user " + userId + " over the period " + startIso + " to " + endIso +
        " (" + currencies.map(function (v) { return v === null ? "(null)" : v; }).join(", ") +
        "). Refusing to sum across mixed currencies — a figure in no single unit is not a total. " +
        "Reporting null and marking it unreadable.");
      unreadable.push("revenueTotal");
      metrics.revenueTotal = null;
    } else {
      // Zero or one distinct currency. A single null counts as a single
      // currency: unspecified throughout is consistent, whatever it means.
      for (var i = 0; i < rows.length; i++) {
        ...
        // this is not defensive padding: `+=` on a string would concatenate.
