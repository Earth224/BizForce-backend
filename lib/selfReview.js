...
/* The model name recorded on the row.
   callAnthropicText returns { text, stopReason } and does NOT report which
   model served the call, so this mirrors the default in its signature
   (server.js:9174) rather than reading it back. That is a real coupling: if
   that default changes and this constant does not, every row written here
   would attribute the narrative to the wrong model — quietly, since nothing
   would fail. A caller that passes its own model should pass the name here
   too, which is what the `modelName` option is for. */
