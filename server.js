...
if (existing) {
const token = jwt.sign({ id: existing.id, email: existing.email }, process.env.JWT_SECRET, { expiresIn: ‘7d’ });
return res.json({ token, user: { id: existing.id, email: existing.email, business_name: existing.business_name }});
}
...
res.json({ token, user: { id: data.id, email: data.email, business_name: data.business_name }});
