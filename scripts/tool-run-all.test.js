...
const ROUTES = process.argv.length > 2 ? process.argv.slice(2) : Object.keys(F);
const results = [];
function rec(route, test, ok, detail) { results.push({ route, test, ok, detail }); }

(async () => {
  for (const path of ROUTES) {
    const fx = F[path];
    const usesModel = fx.model !== null;
    const plan = () => ({ modelText: fx.model });
    let successAfter, successBefore;

    // a) success
    try {
      const h = build(after, path, plan());
      const r = await call(h.handler, fx.body);
      assert.strictEqual(r.status, 200, "status " + r.status + " " + (r.nextErr && r.nextErr.message) + " " + String(JSON.stringify(r.body)).slice(0, 200));
      const w = norm(h.writes);
      assert.strictEqual(w.length, 2, "writes " + w.length);
      assert.strictEqual(w[0].op, "insert");
      assert.strictEqual(w[0].payload.agent_type, fx.agent);
      assert.strictEqual(w[0].payload.task_type, path);
      assert.strictEqual(w[0].payload.status, "processing");
      assert.strictEqual(w[0].payload.user_id, "u1");
      assert.strictEqual(w[0].payload.result, null);
      assert(typeof w[0].payload.prompt === "string" && w[0].payload.prompt.length > 8 && w[0].payload.prompt.length <= 200 && !/[{}\[\]"]/.test(w[0].payload.prompt), "prompt: " + w[0].payload.prompt);
      if (fx.prompt) assert.strictEqual(w[0].payload.prompt, fx.prompt);
      ...
      assert(typeof w[1].payload.result === "string" && w[1].payload.result.length > 20 && !/\[object Object\]/.test(w[1].payload.result));
      ...
      if (usesModel) assert.deepStrictEqual(h.modelCalls, ["POST /api/agents/" + path]); else assert.deepStrictEqual(h.modelCalls, []);
      successAfter = r;
      rec(path, "a", true, w[0].payload.prompt);
    } catch (e) { rec(path, "a", false, e.message); }

    // b) validation failure
    try {
      const h = build(after, path, plan()), h0 = build(before, path, plan());
      const r = await call(h.handler, fx.bad), r0 = await call(h0.handler, fx.bad);
      ...
      rec(path, "b", true, "");
    } catch (e) { rec(path, "b", false, e.message); }

    // c) model throws
    if (usesModel) {
      try {
        const h = build(after, path, Object.assign(plan(), { modelThrows: true }));
        const r = await call(h.handler, fx.body);
        assert(r.nextErr && /model-down/.test(r.nextErr.message), "expected model-down via next");
        const w = norm(h.writes);
        ...
        rec(path, "c", true, "");
      } catch (e) { rec(path, "c", false, e.message); }
    } else rec(path, "c", true, "n/a — no model call on this route");

    // d) every error-after-insert path (502 parse failure) marks failed; also the thrown-error path is c
    if (fx.parseFail !== undefined) {
      try {
        const h = build(after, path, { modelText: fx.parseFail });
        const r = await call(h.handler, fx.body);
        assert.strictEqual(r.status, 502, "expected 502, got " + r.status + " " + (r.nextErr && r.nextErr.message));
        const w = norm(h.writes);
        assert.strictEqual(w.length, 2);
        assert.strictEqual(w[1].payload.status, "failed");
        assert(/could not be read back/.test(w[1].payload.error), w[1].payload.error);
        assert.strictEqual(w[1].payload.completed_at, "2026-09-11T12:00:00.000Z");
        rec(path, "d", true, "502 → failed");
      } catch (e) { rec(path, "d", false, e.message); }
    } else rec(path, "d", true, "n/a — no error path after the insert besides a throw (covered by c)");

    // e) identity minus the two keys
    try {
      assert(successAfter, "no success response from a)");
      const h0 = build(before, path, plan());
      successBefore = await call(h0.handler, fx.body);
      assert.strictEqual(successBefore.status, 200);
      assert.deepStrictEqual(strip(successAfter.body), strip(successBefore.body));
      assert.deepStrictEqual(Object.keys(successAfter.body), Object.keys(strip(successBefore.body)).concat(["task_id", "persisted"]));
      rec(path, "e", true, "");
    } catch (e) { rec(path, "e", false, e.message); }

    // f) completion update error → persisted false + UNPERSISTED line (spot check of the shared helper per route)
    try {
      const h = build(after, path, Object.assign(plan(), { updateError: { message: "update-down" } }));
      const r = await call(h.handler, fx.body);
      assert.strictEqual(r.status, 200);
      assert.strictEqual(r.body.persisted, false);
      assert.strictEqual(h.logs.filter(l => l[1].indexOf("UNPERSISTED TOOL OUTPUT") === 0).length, 1);
      rec(path, "f", true, "");
    } catch (e) { rec(path, "f", false, e.message); }
  }

  // table
  const tests = ["a", "b", "c", "d", "e", "f"];
  const pad = (s, n) => (s + " ".repeat(n)).slice(0, n);
  console.log(pad("route", 34) + tests.map(t => pad(t, 6)).join("") + "title (from the insert)");
  let failures = 0;
  for (const path of ROUTES) {
    const row = tests.map(t => { const r = results.find(x => x.route === path && x.test === t); if (!r) return pad("-", 6); if (!r.ok) failures++; return pad(r.ok ? "PASS" : "FAIL", 6); }).join("");
    const a = results.find(x => x.route === path && x.test === "a");
    console.log(pad(path, 34) + row + (a && a.ok ? a.detail : ""));
  }
  results.filter(r => !r.ok).forEach(r => console.log("  FAIL " + r.route + " " + r.test + ": " + r.detail));
